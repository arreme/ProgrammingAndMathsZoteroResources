{
	...
	"configOptions": {
		...
		"getCollections": true,
		"cached": true,
		"hash": "07fddfbd074ce24b0e675af2084c6b3e04dd3e145331383a28308b6b121ff402"
	},
	"displayOptions": {
		"exportNotes": false,
...
${input}`;
    ...
    for (const bibtex of bib.entries) {
      ...
      id++;
      if (id % 1e3 === 0) await new Promise((resolve) => setTimeout(resolve, 0));
      if (bibtex.key) itemIDS[bibtex.key] = id;
      try {
    ...
    if (errors.length) {
      ...
      for (const err of errors) {
        item.note += "<li>";
        item.note += html(err.error);
        if (err.input) {
          Zotero.debug(`import error: ${err.error}
>>>
${err.input}
<<<`);
          item.note += `<pre>${html(err.input)}</pre>`;
        }
        item.note += "</li>";
      }
      item.note += "</ul>";
